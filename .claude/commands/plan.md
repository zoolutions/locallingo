...
- The chosen design must respect project invariants: non-destructive state sync (`manual` flags survive every rewrite), idempotent newline-terminated state writes, corrupted state raises rather than resets, no version bumps in PRs, app-specific behavior lives in `.locallingo.yml`, API keys never stored or logged, backwards compatibility for existing configs / state files / CLI flags, TDD (specs named before implementation steps), mocked providers in specs.
- Decide the test strategy per the testing rules: unit specs for config/state/validators, `with_app`-fixture specs for Manager workflows, mocked provider for translation paths.
...
<Bullet list: `path/to/file.rb` — why it matters to this change. Include CLI, manager, state, validator, and quality layers as relevant. Self-contained: no references to "as discussed" or this session.>
